...
O state fica em S3 (`soat-tech-challenge-tfstate`) com lock via DynamoDB (`soat-tech-challenge-tfstate-lock`), compartilhado entre execuções de CI/CD. Esses dois recursos são provisionados manualmente uma única vez (bootstrap), fora do Terraform gerenciado por este repo, para evitar o problema de "quem provisiona o backend do próprio backend".
